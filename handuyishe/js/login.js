...
//邮箱注册时 邮箱验证
mail_a.onblur=function Comfirnm_mail(){
	var Mail_num=mail_a.value;
	var pattern=/^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+/;
	if(Mail_num!=""&&pattern.test(Mail_num)){
		mail_A.innerHTML="您输入正确！";
	}
	else{
		mail_A.innerHTML="您邮箱格式不正确";
		$("#mail_A").css("color","red");
	}
}
...
function Comfirm_code(){
	...
	var s3=ran();
	var s4=ran();
	Code_a.innerText=""+str[s1]+str[s2]+str[s3]+str[s4];
	Code_at.innerText=""+str[s1]+str[s2]+str[s3]+str[s4];
	console.log( Code_a )
	console.log(Code_a.innerText);
}
Comfirm_code();
//验证邮箱密码是否一致
mail_d.onblur=function check_Code(){
     if((mail_d.value).toUpperCase()===(Code_at.innerText).toUpperCase()){
	     return true;
      }else{
      	return false;
      }
...
//密码是否与确认密码一致
mail_c.onblur=function check_Code1(){
     if((mail_b.value).toUpperCase()==(mail_c.value).toUpperCase()){
	     return true;
      }else{
      	return false;
      }
...
//存取cookie
var phoneNum=phone_num.value;
var phonePass=_code.value;
var mailNum=mail_a.value;
var mailPass=mail_b.value;
  if(phoneNum!=""&&code!=""&&check_Code()){
     setCookie("phone",{"num":"phoneNum","password":"phonePass"},10);
     setCookie("mail",{"num":"mailNum","password":"mailPass"},10);
  }
 console.log((mail_d.value).toUpperCase());
 console.log((Code_at.innerText).toUpperCase());
